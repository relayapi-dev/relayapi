...
RelayAPI is a unified social media API for posting to 22 platforms via a single API. TypeScript monorepo deployed on Cloudflare Workers.
...
- **Git**: Git write operations are prohibited by default, but the user may authorize them for the current task in ordinary language. No fixed phrase or magic wording is required. A direct request such as “commit”, “push”, or “commit and push” authorizes those named operations in the current repository, and a short affirmative reply such as “yes”, “go”, or “do it” counts when it directly answers a clearly stated proposal that names the Git operations and scope. Authorization is limited to the current task and does not carry over to later tasks.
  - Authorization to “commit” includes `git add` and `git commit`, but never implies permission to push unless push is also requested or clearly affirmed.
  - `git push`, `git commit --amend`, force operations, and destructive/history-changing commands must be explicitly requested in ordinary language or clearly affirmed after the assistant states the exact operation and scope; they do not require a prescribed authorization sentence.
  - Never run `git reset --hard`, `git clean`, `git checkout --`, or `git restore` without specific confirmation immediately before execution.
  - Before writing Git state, inspect `git status` and the relevant diff. Afterward, report exactly what changed.
