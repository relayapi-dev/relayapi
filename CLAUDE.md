...
- **Dependency catalog (Bun)**: Shared dependency versions are centralized in the Bun catalog under `workspaces.catalog` in the root `package.json`. The `private`, Bun-installed packages — `apps/api`, `apps/app`, `apps/docs`, `packages/auth`, `packages/db` — reference catalog versions with `"<pkg>": "catalog:"` instead of literal ranges. When adding or changing a dependency in one of these, set the version in the root catalog and use `"catalog:"` in the package (add the catalog entry first, or `bun install` fails to resolve the ref). Do **not** use `catalog:` in `packages/sdk`, `packages/mcp`, or `packages/integrations/*` — they are published/built with npm, which does not understand the `catalog:` protocol; keep literal versions there. Run `bun run deps:upgrade` to bump catalog entries to their latest npm versions (`--dry-run` to preview, `--force` to include held-back packages), then `bun install`. That script holds back a few packages (documented with reasons in its `HELD` map — e.g. `typescript`, `@tsparticles/*`, the `three`/`postprocessing` stack) whose latest majors need a deliberate migration; upgrade those by hand and re-run `bun run typecheck`.
