...
Do not hardcode an internal credential in a Snippet. Use a Worker with a secret
binding for authenticated upstream requests, and validate caller authorization
before attaching that credential. Snippet source and versioned configuration are
not a secret store.
