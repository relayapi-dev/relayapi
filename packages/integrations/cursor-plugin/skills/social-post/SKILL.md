...
0. **Check authentication** — Verify the RelayAPI API key is configured (available as `RELAYAPI_API_KEY` env var). If missing, guide the user to set it up:
   ...
   - Set the environment variable: `export RELAYAPI_API_KEY="rlay_live_..."`
   - Do NOT proceed until the key is configured. Do NOT ask the user to paste the key in chat.
1. **Ask the user** what they want to post and to which platforms or workspaces if not already specified
...
3. **Resolve targets** — the user may refer to platforms by name ("Twitter"), workspaces ("Marketing"), or specific accounts. Use:
   ...
   - Workspace ID (e.g. `"ws_abc123"`) → all accounts in the workspace. Use `GET /v1/workspaces` to find the workspace ID by name.
...
5. **Customize per platform** if the content needs adjustment — use `target_options` for per-target content:
   - Shorter text for Twitter (280 chars), Bluesky (300), Threads/Mastodon (500), and Pinterest (800)
...
const marketing = workspaces.data.find(workspace => workspace.name === "Marketing");
...
Media types are `image`, `video`, `gif`, `document`, and `audio`. WhatsApp audio
messages require a digits-only `target_options.whatsapp.to` recipient and no
caption content.
...
- When the user mentions a workspace name, look it up via `GET /v1/workspaces` and use the `ws_*` ID as a target
