...
1. If no content was provided, ask me what I want to post and to which platforms or workspaces
2. Check my connected accounts via `GET /v1/accounts` and workspaces via `GET /v1/workspaces`
3. Resolve my targets:
   - If I mentioned a platform name (e.g. "Twitter") → use it directly
   - If I mentioned a workspace name (e.g. "Marketing") → find the `ws_*` ID from workspaces
