---
title: Quickstart
description: Get started with the RelayAPI unified social media API in minutes.
icon: Rocket
---

## Overview

RelayAPI is a unified API that lets you publish to 22 social, messaging, SMS, and newsletter channels with a single API call. Connect your accounts, create posts, and publish everywhere through one consistent interface.

## Authentication

Hosted signup is currently invite-only. Existing hosted users can [sign in](https://relayapi.dev/login); everyone else can [request early access](mailto:support@relayapi.dev?subject=RelayAPI%20early%20access) or follow the [self-hosting guide](/guides/self-hosting). After you have an instance, create an API key from its dashboard.

### Getting Your API Key

1. Log in to your hosted or self-hosted RelayAPI dashboard
2. Go to **Settings > API Keys**
3. Click **Create API Key**
4. Copy the key immediately — you won't be able to see it again

### Key Format

API keys are prefixed with `rlay_live_` for production or `rlay_test_` for testing. Keys are stored as SHA-256 hashes and are only shown once at creation.

## Choose a client

The TypeScript SDK is the canonical current client. Go and Java clients are generated and released separately. RelayAPI does not currently publish a Python package; the Python examples below use the REST API directly.

<Tabs items={['TypeScript', 'Python', 'Go', 'Java', 'curl']}>
<Tab value="TypeScript">
```bash
npm install @relayapi/sdk
```
</Tab>
<Tab value="Python">
No package installation is required when using Python's standard library. See [Python REST and OpenAPI](/sdks/python) for a reusable helper and client-generation guidance.
</Tab>
<Tab value="Go">
```bash
go get github.com/relayapi-dev/relay-go
```
</Tab>
<Tab value="Java">
```bash
# Gradle
implementation("dev.relayapi:relay-java:0.0.1")
```
</Tab>
<Tab value="curl">
```bash
# No installation needed — use curl directly
```
</Tab>
</Tabs>

### Set Up the Client

<Tabs items={['TypeScript', 'Python', 'Go', 'Java', 'curl']}>
<Tab value="TypeScript">
```typescript
import Relay from '@relayapi/sdk';

const client = new Relay(); // reads RELAY_API_KEY from environment
```
</Tab>
<Tab value="Python">
```python
import json
import os
from urllib.request import Request, urlopen

base_url = os.getenv("RELAY_BASE_URL", "https://api.relayapi.dev").rstrip("/")
api_key = os.environ["RELAY_API_KEY"]

def relay_request(method, path, payload=None):
    body = None if payload is None else json.dumps(payload).encode("utf-8")
    request = Request(
        f"{base_url}{path}",
        data=body,
        method=method,
        headers={
            "Authorization": f"Bearer {api_key}",
            **({"Content-Type": "application/json"} if body is not None else {}),
        },
    )
    with urlopen(request, timeout=60) as response:
        return json.load(response)
```
</Tab>
<Tab value="Go">
```go
import relaygo "github.com/relayapi-dev/relay-go"

client := relaygo.NewClient() // reads RELAY_API_KEY from environment
```
</Tab>
<Tab value="Java">
```java
import dev.relayapi.api.client.RelayClient;
import dev.relayapi.api.client.okhttp.RelayOkHttpClient;

RelayClient client = RelayOkHttpClient.fromEnv(); // reads RELAY_API_KEY from environment
```
</Tab>
<Tab value="curl">
```bash
# Set your API key as an environment variable
export RELAY_API_KEY="rlay_live_your_api_key"
```
</Tab>
</Tabs>

## Connect a Social Account

Before posting, connect at least one social media account through the dashboard or via the API.

<Tabs items={['TypeScript', 'Python', 'Go', 'Java', 'curl']}>
<Tab value="TypeScript">
```typescript
const flow = await client.connect.startOAuthFlow('twitter');
// Redirect user to flow.auth_url to authorize
```
</Tab>
<Tab value="Python">
```python
flow = relay_request("GET", "/v1/connect/twitter")
# Redirect user to flow["auth_url"] to authorize
```
</Tab>
<Tab value="Go">
```go
flow, err := client.Connect.StartOAuthFlow(context.TODO(), "twitter", relaygo.ConnectStartOAuthFlowParams{})
// Redirect user to flow.AuthURL to authorize
```
</Tab>
<Tab value="Java">
```java
ConnectStartOAuthFlowResponse flow = client.connect().startOAuthFlow("twitter");
// Redirect user to flow.authUrl() to authorize
```
</Tab>
<Tab value="curl">
```bash
curl https://api.relayapi.dev/v1/connect/twitter \
  -H "Authorization: Bearer $RELAY_API_KEY"
```
</Tab>
</Tabs>

### Available Platforms

See [Platforms Overview](/platforms) for the full list of supported platforms and their capabilities.

## Create and Publish a Post

<Tabs items={['TypeScript', 'Python', 'Go', 'Java', 'curl']}>
<Tab value="TypeScript">
```typescript
const post = await client.posts.create({
  content: 'Hello from RelayAPI!',
  targets: ['twitter', 'linkedin'],
  scheduled_at: 'now',
});

console.log(post.id);     // "post_abc123"
console.log(post.status); // "published"
```
</Tab>
<Tab value="Python">
```python
post = relay_request(
    "POST",
    "/v1/posts",
    {
        "content": "Hello from RelayAPI!",
        "targets": ["twitter", "linkedin"],
        "scheduled_at": "now",
    },
)

print(post["id"])      # "post_abc123"
print(post["status"])  # "published"
```
</Tab>
<Tab value="Go">
```go
post, err := client.Posts.New(context.TODO(), relaygo.PostNewParams{
    Content:     relaygo.F("Hello from RelayAPI!"),
    Targets:     relaygo.F([]string{"twitter", "linkedin"}),
    ScheduledAt: relaygo.F("now"),
})

fmt.Println(post.ID)     // "post_abc123"
fmt.Println(post.Status) // "published"
```
</Tab>
<Tab value="Java">
```java
PostCreateResponse post = client.posts().create(PostCreateParams.builder()
    .content("Hello from RelayAPI!")
    .addTarget("twitter")
    .addTarget("linkedin")
    .scheduledAt("now")
    .build());

System.out.println(post.id());     // "post_abc123"
System.out.println(post.status()); // "published"
```
</Tab>
<Tab value="curl">
```bash
curl -X POST https://api.relayapi.dev/v1/posts \
  -H "Authorization: Bearer $RELAY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "content": "Hello from RelayAPI!",
    "targets": ["twitter", "linkedin"],
    "scheduled_at": "now"
  }'
```
</Tab>
</Tabs>

### Posting to Multiple Platforms

Target multiple accounts in a single API call. RelayAPI handles platform-specific formatting automatically.

### Scheduling Posts

Schedule posts for future publishing by adding `scheduled_at` and `timezone`:

<Tabs items={['TypeScript', 'Python', 'Go', 'Java', 'curl']}>
<Tab value="TypeScript">
```typescript
const post = await client.posts.create({
  content: 'Scheduled post!',
  targets: ['twitter'],
  scheduled_at: '2024-01-16T12:00:00',
  timezone: 'America/New_York',
});
```
</Tab>
<Tab value="Python">
```python
post = relay_request(
    "POST",
    "/v1/posts",
    {
        "content": "Scheduled post!",
        "targets": ["twitter"],
        "scheduled_at": "2024-01-16T12:00:00",
        "timezone": "America/New_York",
    },
)
```
</Tab>
<Tab value="Go">
```go
post, err := client.Posts.New(context.TODO(), relaygo.PostNewParams{
    Content:     relaygo.F("Scheduled post!"),
    Targets:     relaygo.F([]string{"twitter"}),
    ScheduledAt: relaygo.F("2024-01-16T12:00:00"),
    Timezone:    relaygo.F("America/New_York"),
})
```
</Tab>
<Tab value="Java">
```java
PostCreateResponse post = client.posts().create(PostCreateParams.builder()
    .content("Scheduled post!")
    .addTarget("twitter")
    .scheduledAt("2024-01-16T12:00:00")
    .timezone("America/New_York")
    .build());
```
</Tab>
<Tab value="curl">
```bash
curl -X POST https://api.relayapi.dev/v1/posts \
  -H "Authorization: Bearer $RELAY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "content": "Scheduled post!",
    "targets": ["twitter"],
    "scheduled_at": "2024-01-16T12:00:00",
    "timezone": "America/New_York"
  }'
```
</Tab>
</Tabs>

## Supported Platforms

| Platform | Post | Schedule | Media | Analytics |
|----------|------|----------|-------|-----------|
| Twitter/X | ✅ | ✅ | ✅ | ✅ |
| Instagram | ✅ | ✅ | ✅ | ✅ |
| Facebook | ✅ | ✅ | ✅ | ✅ |
| LinkedIn | ✅ | ✅ | ✅ | ✅ |
| TikTok | ✅ | ✅ | ✅ | ✅ |
| YouTube | ✅ | ✅ | ✅ | ✅ |
| Pinterest | ✅ | ✅ | ✅ | ✅ |
| Reddit | ✅ | ✅ | ✅ | ✅ |
| Bluesky | ✅ | ✅ | ✅ | ✅ |
| Threads | ✅ | ✅ | ✅ | ✅ |
| Telegram | ✅ | ✅ | ✅ | — |
| Snapchat | ✅ | ✅ | ✅ | — |
| Discord | ✅ | ✅ | ✅ | — |
| Slack | ✅ | ✅ | ✅ | — |
| Mastodon | ✅ | ✅ | ✅ | — |
| WhatsApp | ✅ | ✅ | ✅ | ✅ |
| Google Business | ✅ | ✅ | ✅ | ✅ |
| SMS | ✅ | ✅ | ✅ | — |
| Beehiiv | ✅ | ✅ | Varies | — |
| Kit (ConvertKit) | ✅ | ✅ | Varies | — |
| Mailchimp | ✅ | ✅ | Varies | — |
| Listmonk | ✅ | ✅ | Varies | — |

The Analytics column means a live native account adapter. Stored cross-platform post metrics may still be available when RelayAPI has collected a snapshot. Provider account type, review, scopes, and media policy can narrow any capability; consult the relevant [platform guide](/platforms) before depending on it.

## What's Next?

- [Platforms Overview](/platforms) — Learn about platform-specific features
- [API Reference](/api-reference) — Full endpoint documentation
- [Guides](/guides/connecting-accounts) — Step-by-step guides
