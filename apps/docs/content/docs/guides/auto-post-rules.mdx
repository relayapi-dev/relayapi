...
This is a **Pro** feature. `workspace_id` follows the organization's **Require
Workspace ID** policy: when the setting is disabled, omit it to create an
organization-scoped rule; when the setting is enabled, an independent rule
must include an active, authorized workspace. Merely having workspaces does not
