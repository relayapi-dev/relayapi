...
# Test the connection with a read-only provider probe (no link is created)
...
console.log(test.success, test.error);
...
| `POST` | `/v1/short-links/test` | Read-only provider credential probe |
