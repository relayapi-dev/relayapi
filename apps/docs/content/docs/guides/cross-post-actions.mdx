...
| `processing` | RelayAPI has claimed the action and is preparing the provider request |
| `executing` | The provider request is in progress |
| `retry` | A retryable failure occurred and another attempt is scheduled |
| `executed` | Action completed successfully |
| `failed` | Action failed (e.g., platform error, account disconnected) |
| `unknown` | The provider outcome could not be proven and requires reconciliation |
...
const { data } = await client.crossPostActions.listByPost("post_abc123");

// Cancel a pending action
await client.crossPostActions.cancel("cpa_xyz789");
```
