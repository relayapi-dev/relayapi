...
| Thumbnails per post | 1, attached to the single video |
...
## Comment moderation and video ratings

With `youtube.force-ssl`, RelayAPI can edit an owned comment, set moderation to
`approve`, `hold_for_review`, or `reject`, and rate a video with `like`,
`dislike`, or `clear_rating`. Reauthorize an older connection that lacks the
scope. Every provider write is idempotent and durable; see [Published Edits and
Social Actions](/guides/published-edits-and-social-actions).

## Automations

<Callout type="warn">
RelayAPI does not currently register executable YouTube-specific comment, live-chat, or new-video triggers, nor YouTube reply/moderation send nodes. YouTube PubSub notifications are not yet persisted and dispatched into the automation runtime. Do not build workflows against the previously proposed `youtube_*` node names; they are not part of the current API contract.
</Callout>

You can still schedule YouTube publication with the normal posts API and invoke platform-independent RelayAPI automation actions where their documented inputs apply.
