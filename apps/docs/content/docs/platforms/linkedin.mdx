...
Post to a company page instead of a personal profile by targeting the RelayAPI account created when that organization was selected during the LinkedIn connection flow. The connected member or organization URN is authoritative for every post; a post cannot use an option to redirect that OAuth token to another company.
...
| `organization_urn` | string | Deprecated identity assertion only. If supplied, it must exactly match the connected member/organization URN; it never retargets the post. |
...
| `ORGANIZATION_URN_MISMATCH` | Deprecated `organization_urn` does not equal the connector-selected identity | Target the correct connected account and remove the compatibility field; RelayAPI rejects the mismatch before provider I/O. |
