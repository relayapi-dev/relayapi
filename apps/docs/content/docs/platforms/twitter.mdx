...
RelayAPI can edit a published X Post's text, edit an owned reply, hide/unhide a
reply, and like/unlike a Post. An X edit creates a replacement Post ID; RelayAPI
updates the target and retains the old ID in edit history. Every action requires
an idempotency key and produces a durable operation. See [Published Edits and
Social Actions](/guides/published-edits-and-social-actions) for the typed SDK
calls, exact action matrix, and unknown-outcome handling.
