...
| `schedule_time` | string | No | ISO 8601 delivery time. RelayAPI rounds forward to Mailchimp's next quarter-hour boundary; omission requests an immediate send. |

Relay media attachments are not supported for Mailchimp campaigns. Include
