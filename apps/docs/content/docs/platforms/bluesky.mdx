...
| Image max size | 2,000,000 bytes per image (about 1.91 MiB) |
...
<Callout type="warn">
Bluesky has a **300-character hard limit** — this is the #1 cause of failed posts when cross-posting from other platforms. If you are cross-posting from Twitter (280), LinkedIn (3,000), or Facebook (63,000), always use `target_options.bluesky.content` to provide a version under 300 characters. The canonical image-embed Lexicon limits each image blob to **2,000,000 decimal bytes** (about 1.91 MiB); RelayAPI rejects a larger source instead of compressing it silently. Bluesky uses **App Passwords** instead of OAuth.
</Callout>
...
Each image must be at most 2,000,000 bytes. RelayAPI does not silently recompress the source; prepare the image before publishing.
...
| Max file size | 2,000,000 bytes per image (about 1.91 MiB) |
...
| Image too large | Image exceeds 2,000,000 bytes | Compress the image below the exact byte limit before upload. |
...
- **2,000,000-byte image limit** — this is a decimal-byte Lexicon limit, not 2 MiB. Most phone photos still need compression.
- **No silent image recompression** — RelayAPI fails closed when the source exceeds the blob limit.
