...
// PUT once using every exact header returned with the presign
...
const confirmed = await client.media.confirm({ storage_key: storageKey });
if (confirmed.id !== presign.id) throw new Error('Confirmed the wrong media intent');
