...
async function fetchMarkdownContent(markdownUrl: string): Promise<string> {
  const cached = cache.get(markdownUrl);
...
export function ViewOptionsPopover({
  ...
  const handleOpenInAI = async (
    buildUrl: (content: string) => string,
  ) => {
    ...
    try {
      ...
      const q = `Here is the full documentation for ${pageUrl}:\n\n${content}\n\nI want to ask questions about it.`;
      ...
      <PopoverContent className="flex flex-col">
        ...
        {aiItems.map((item) => (
          <button
            ...
            key={item.title}
            disabled={isLoading}
            onClick={() => handleOpenInAI(item.buildUrl)}
            className="text-sm p-2 rounded-lg inline-flex items-center gap-2 hover:text-fd-accent-foreground hover:bg-fd-accent [&_svg]:size-4 text-left disabled:opacity-50"
          >
            {item.icon}
            {item.title}
            <ExternalLinkIcon className="text-fd-muted-foreground size-3.5 ms-auto" />
