...
export function AdsPage({
	...
	const adsQuery = {
		...
		...(adSource !== "all"
			? { source: adSource === "created" ? "internal" : "external" }
			: {}),
	};
	const {
		data: ads,
		...
		refetch: adsRefetch,
	} = usePaginatedApi<Ad>(activeTab === "ads" ? "ads" : null, {
		query: adsQuery,
	});
	...
	const {
		...
		loadMore: campaignsLoadMore,
		loadingMore: campaignsLoadingMore,
		refetch: campaignsRefetch,
	} = usePaginatedApi<Campaign>(
		activeTab === "campaigns" ? "ads/campaigns" : null,
	...
	const {
		...
		hasMore: audiencesHasMore,
		loadMore: audiencesLoadMore,
		loadingMore: audiencesLoadingMore,
	...
	const { mutate: _syncAccount, loading: _syncing } = useMutation(
		"ads/accounts",
		"POST",
	);
		...
		async (accountId: string) => {
			...
			try {
				...
				setAccountActionError(
					err instanceof Error ? err.message : "Failed to sync ad account.",
				);
			}
		},
			...
			{activeTab === "ads" && (
              ...
              is kept on small screens); shown from sm: upward. */}
					...
					{adsLoading ? (
						...
						<>
							<motion.div
								className="overflow-x-auto scrollbar-hide rounded-[12px] border border-border bg-card"
								variants={stagger}
								initial="hidden"
								animate="visible"
							>
								<table className="w-full text-sm">
									<thead>
										<tr className="border-b border-border bg-muted/40 text-xs font-medium text-muted-foreground">
											...
											<th className="px-4 py-2.5 text-right hidden lg:table-cell">
												Budget
											</th>
										...
										{ads.map((ad, i) => (
											<motion.tr
												...
												className={cn(
													...
													i !== ads.length - 1 && "border-b border-border",
												)}
											>
												...
												<td className="px-4 py-3 hidden md:table-cell">
													<Badge config={platformConfig} value={ad.platform} />
												</td>
												<td className="px-4 py-3">
													<Badge config={statusConfig} value={ad.status} />
												</td>
												<td className="px-4 py-3 text-xs text-muted-foreground text-right hidden lg:table-cell">
													{ad.daily_budget_cents
														? `${formatBudget(ad.daily_budget_cents)}/day`
														: formatBudget(ad.lifetime_budget_cents)}
												</td>
												...
												<td className="px-4 py-3 text-right">
													<div className="flex items-center justify-end gap-1">
														...
														{ad.status === "active" && (
															<button
																type="button"
																onClick={() => handleUpdateAd(ad.id, "paused")}
																className="p-1 rounded hover:bg-accent transition-colors text-muted-foreground hover:text-foreground"
																title="Pause"
															>
														...
														{ad.status === "paused" && (
															<button
																type="button"
																onClick={() => handleUpdateAd(ad.id, "active")}
																className="p-1 rounded hover:bg-accent transition-colors text-muted-foreground hover:text-foreground"
																title="Resume"
															>
              ...
              kept on small screens. */}
					...
					{campaignsLoading ? (
						...
						<>
									...
									<tbody>
										{campaigns.map((camp, i) => (
											...
											>
												...
												<td className="px-4 py-3">
													<Badge config={statusConfig} value={camp.status} />
												</td>
												<td className="px-4 py-3 text-xs text-muted-foreground text-right hidden lg:table-cell">
													{camp.daily_budget_cents
														? `${formatBudget(camp.daily_budget_cents, camp.currency)}/day`
														: formatBudget(
																camp.lifetime_budget_cents,
																camp.currency,
															)}
												</td>
													...
													<div className="flex items-center justify-end gap-1">
														{(camp.status === "active" ||
															...
															<button
																type="button"
																onClick={() =>
																	handleToggleCampaign(camp.id, camp.status)
																}
																className="p-1 rounded hover:bg-accent transition-colors text-muted-foreground hover:text-foreground"
																title={
							...
							<LoadMore
								hasMore={campaignsHasMore}
								loading={campaignsLoadingMore}
								onLoadMore={campaignsLoadMore}
			...
			{activeTab === "audiences" && (
				<>
					...
					{!selectedAdAccountId ? (
						<div className="rounded-[12px] border border-dashed border-border p-12 text-center">
							<p className="text-sm text-muted-foreground">
								Select an ad account to view audiences
							</p>
						</div>
					) : audiencesError ? (
						<div className="rounded-[12px] border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
						...
						<>
									...
									<tbody>
										{audiences.map((aud, i) => (
											...
											>
												...
												<td className="px-4 py-3 hidden md:table-cell">
													<Badge config={audienceTypeConfig} value={aud.type} />
												</td>
												<td className="px-4 py-3 hidden lg:table-cell">
													<Badge config={platformConfig} value={aud.platform} />
												</td>
												<td className="px-4 py-3 text-xs text-muted-foreground text-right hidden md:table-cell">
							...
							<LoadMore
								hasMore={audiencesHasMore}
								loading={audiencesLoadingMore}
								onLoadMore={audiencesLoadMore}
											...
											>
												<td className="px-4 py-3">
													...
													<span className="text-[10px] text-muted-foreground font-mono">
														{acc.platform_ad_account_id}
													</span>
												</td>
												<td className="px-4 py-3 hidden md:table-cell">
													<Badge config={platformConfig} value={acc.platform} />
												</td>
		...
		>
			<DialogContent className="max-w-md">
				<DialogHeader>
					...
					<DialogDescription>
						One email or phone number per line (max 10,000). Data is SHA-256
