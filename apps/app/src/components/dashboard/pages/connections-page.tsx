...
export function ConnectionsPage({
	...
	const {
		...
		setData: setAccounts,
	} = usePaginatedApi<Account>(activeTab === "accounts" ? "accounts" : null, {
	...
	// Health dialog state
	const [healthDialogAccount, setHealthDialogAccount] = useState<{
		id: string;
		platform: string;
		username: string | null;
		display_name: string | null;
	...
	const createWorkspaceMutation = useMutation<WorkspaceItem>(
		"workspaces",
		"POST",
	);
	...
	const handleMoveToWorkspace = async () => {
		...
		setMoveError(null);
		const workspaceId =
			moveWorkspaceId === "__ungrouped" ? null : moveWorkspaceId;
		try {
			...
			if (!res.ok) {
				...
				setMoveError(
					err?.error?.message || err?.message || `Error ${res.status}`,
				);
				return;
			}
	...
	const handleArchiveWorkspace = async (workspace: WorkspaceItem) => {
		...
		if (res.ok) {
			setGroups((prev) => prev.filter((g) => g.id !== workspace.id));
		}
	};
	...
	return (
		<div className="space-y-5 pb-16">
											...
											<div className="flex items-start justify-between">
												<div className="relative">
													...
													{acc.avatar_url && (
														...
														>
															{platformAvatars[platform]?.slice(0, 1) ||
												...
												<DropdownMenu>
													<DropdownMenuTrigger asChild>
														<button
															type="button"
															className="rounded-md p-1.5 opacity-0 group-hover:opacity-100 hover:bg-muted transition-all"
														>
													...
													<DropdownMenuContent align="end" className="w-44">
														{[
															...
															<DropdownMenuItem
																onClick={() => {
																	window.location.href = `/app/connections/${acc.id}?tab=default-reply`;
																}}
															>
														...
														{(() => {
															...
															if (
																...
																if (platform === "instagram")
																	reconnectParams.set("method", "direct");
																if (acc.workspace?.id)
																	reconnectParams.set(
																		"workspace_id",
																		acc.workspace.id,
																...
																const reconnectQuery =
																	reconnectParams.toString();
																const reconnectUrl = `/app/connect/start/${platform}${reconnectQuery ? `?${reconnectQuery}` : ""}`;
																return (
																	<DropdownMenuItem
																		onClick={() => {
			...
			{activeTab === "connect" && (
				<div className="space-y-3">
					<p className="text-xs text-muted-foreground">
						{workspaceFilterId && workspaceFilterId !== "__ungrouped"
							? "New connections will be assigned to the selected workspace."
							: "New connections will use organization scope unless Require Workspace ID is enabled."}
					</p>
					<PlatformGrid
						onConnected={accountsRefetch}
						workspaceId={
							workspaceFilterId && workspaceFilterId !== "__ungrouped"
								? workspaceFilterId
								: undefined
						...
						>
							...
							<div className="flex gap-2">
								<Button
									...
									disabled={
										!newWorkspaceName.trim() || createWorkspaceMutation.loading
									}
									onClick={handleCreateWorkspace}
								>
									{createWorkspaceMutation.loading ? (
										...
										<div className="flex items-start justify-between">
											<div className="flex items-start gap-3">
												...
												<div>
													...
													<p className="text-xs text-muted-foreground mt-1">
														{group.account_count} account
														{group.account_count !== 1 ? "s" : ""} &middot;
														Created{" "}
											...
											<button
												...
												onClick={() => handleArchiveWorkspace(group)}
					...
					<>
						...
						>
							...
							{healthItems.map((item, i) => {
								...
								const expiresAt = item.token_expires_at
									? new Date(item.token_expires_at)
									: null;
								const now = new Date();
								...
								const canAnalytics = hasAnalyticsCapability(
									platform,
									item.scopes,
								);
						...
						<AccountHealthDialog
							...
							onOpenChange={(open) => {
								if (!open) setHealthDialogAccount(null);
							}}
						/>
					</>
				))}
								...
								{logs.map((log) => {
									...
									return (
										...
										>
											...
											<span
												className={cn(
													"shrink-0 rounded px-1.5 py-0.5 text-center text-[10px] font-semibold uppercase whitespace-nowrap",
													eventStyles[event] || "text-blue-400",
													eventBg[event] || "bg-blue-400/10",
												)}
											>
												{event.replace("_", " ")}
											</span>
			...
			>
				<DialogContent showCloseButton={false} className="sm:max-w-sm">
					...
					<DialogFooter>
						...
						<Button size="sm" disabled={moving} onClick={handleMoveToWorkspace}>
