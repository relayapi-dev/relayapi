import { Loader2, Megaphone, Plus, Rss } from "lucide-react";
...
function broadcastStatusBadge(status: BroadcastResponse["status"]) {
	...
	const map: Record<string, { label: string; classes: string }> = {
		...
		partially_failed: {
			label: "Partial",
			classes: "text-foreground bg-accent",
		},
		requires_attention: {
			label: "Needs attention",
			classes: "text-amber-700 bg-amber-500/10 dark:text-amber-300",
		},
...
export function CampaignsPage({
	...
	} = usePaginatedApi<BroadcastResponse>(
		activeTab === "broadcasts" ? "broadcasts" : null,
		{
			query: filterQuery,
		},
	);
	...
	} = usePaginatedApi<AutoPostRuleResponse>(
		activeTab === "auto-post" ? "auto-post-rules" : null,
		{
			query: filterQuery,
		},
	);
	...
	const [editingAutoPost, setEditingAutoPost] =
		useState<AutoPostRuleResponse | null>(null);
											...
											>
												...
												<td className="px-4 py-3 text-xs text-muted-foreground hidden md:table-cell">
													<span className="font-mono">
														{truncate(r.feed_url, 40)}
													</span>
												</td>
												<td className="px-4 py-3">
													{autoPostStatusBadge(r.status)}
												</td>
												<td className="px-4 py-3 text-xs text-muted-foreground hidden lg:table-cell">
												...
												<td className="px-4 py-3 text-xs text-muted-foreground text-right hidden sm:table-cell">
													{r.last_processed_at ? (
														formatDate(r.last_processed_at)
													) : (
