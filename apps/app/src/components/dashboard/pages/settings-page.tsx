...
interface WorkspaceScopeBlocker {
	resource_type: string;
	count: number;
}

interface OrgSettingsErrorBody {
	error?: {
		code?: string;
		message?: string;
		details?: {
			blockers?: WorkspaceScopeBlocker[];
...
const NOTIFICATION_TYPES: {
	...
	},
	{
		...
		label: "Usage Alerts",
		description:
			"Receive warnings when approaching or reaching your plan limits",
	},
	{
		key: "streakWarnings",
		label: "Streak Warnings",
		description:
			"Get notified when your posting streak is about to end or has ended",
	},
	{
		key: "weeklyDigest",
		label: "Weekly Digest",
...
export function SettingsPage({
	...
	const handleRemoveLogo = async () => {
		...
		try {
			...
			const { error } = await orgClient.update({
				data: { logo: null as unknown as string },
			});
	...
	const handleDeleteOrg = async () => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					body?.error?.message || "Failed to delete organization",
				);
			}

			// The API clears the database session, but Better Auth's signed cookie
			// cache can retain the deleted active organization for up to five
			// minutes. Best-effort switch to another membership (or clear it) before
			// navigating so the next request cannot reuse that stale tenant ID.
			try {
				const { data: remainingOrganizations } = await orgClient.list();
				await orgClient.setActive({
					organizationId: remainingOrganizations?.[0]?.id ?? null,
				});
			} catch {
				// Middleware revalidates membership and handles a stale cookie safely.
			}
	...
	const [orgSettingsRevision, setOrgSettingsRevision] = useState<number | null>(
		null,
	);
	...
	const [orgSettingsBlockers, setOrgSettingsBlockers] = useState<
	...
	useEffect(() => {
		fetch("/api/org-settings")
			...
			.then((data) => {
				if (data?.data) {
					setRequireWorkspaceId(data.data.require_workspace_id);
					setOrgSettingsRevision(data.data.revision);
				}
			})
			.catch((error) => {
				setOrgSettingsError(
	...
	const toggleRequireWorkspaceId = useCallback(async () => {
		if (orgSettingsRevision === null || orgSettingsSaving) return;
		const newValue = !requireWorkspaceId;
		setOrgSettingsSaving(true);
		setOrgSettingsError("");
		setOrgSettingsBlockers([]);
		try {
			const response = await fetch("/api/org-settings", {
				method: "PATCH",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					require_workspace_id: newValue,
					expected_revision: orgSettingsRevision,
				}),
			});
			const payload = (await response.json().catch(() => null)) as
				| { data?: { require_workspace_id: boolean; revision: number } }
				| OrgSettingsErrorBody
				| null;

			if (!response.ok) {
				const error = (payload as OrgSettingsErrorBody | null)?.error;
				if (error?.code === "ORG_SETTINGS_VERSION_CONFLICT") {
					if (typeof error.details?.require_workspace_id === "boolean") {
						setRequireWorkspaceId(error.details.require_workspace_id);
					}
					if (typeof error.details?.current_revision === "number") {
						setOrgSettingsRevision(error.details.current_revision);
					}
				}
				setOrgSettingsBlockers(error?.details?.blockers ?? []);
				setOrgSettingsError(
			...
			if (!data) throw new Error("The settings response was incomplete");
			setRequireWorkspaceId(data.require_workspace_id);
			setOrgSettingsRevision(data.revision);
		} catch (error) {
			setOrgSettingsError(
				error instanceof Error
					? error.message
	...
	}, [orgSettingsRevision, orgSettingsSaving, requireWorkspaceId]);
	...
	const [slTestResult, setSlTestResult] = useState<{
		success: boolean;
	...
	const [sigPosition, setSigPosition] = useState<"append" | "prepend">(
		"append",
	);
						...
						{prefsLoading ? (
							...
							<div className="divide-y divide-border">
								...
								{NOTIFICATION_TYPES.map(
									({ key, label, description, emailAlwaysOn }) => (
										<div
											key={key}
					...
					<div className="rounded-sm border border-border bg-card overflow-hidden">
						...
						{slLoading ? (
							...
							<div className="p-5 space-y-4">
										...
										{(["always", "ask", "never"] as const).map((mode) => (
											...
											>
												...
												<div>
													...
													<p className="text-[11px] text-muted-foreground">
														{mode === "always" &&
															"Automatically shorten all URLs in posts"}
														{mode === "ask" &&
															"Show a toggle in the post dialog to shorten URLs"}
										...
										<div className="flex items-center gap-2">
											<Button
												...
												disabled={
													slTesting ||
													!slProvider ||
													(slProvider !== "relayapi" && !slHasKey && !slApiKey)
												}
											>
												{slTesting ? (
								...
								{slError && (
									<p className="text-xs text-destructive">{slError}</p>
								)}
			...
			{activeTab === "general" && (
				<>
					...
					<div className="rounded-sm border border-border bg-card overflow-hidden">
							...
							<div className="p-5 space-y-4">
								...
								<div className="space-y-3 rounded-md border border-dashed border-border p-3">
									...
									<div className="flex items-center gap-4">
										<Select
											value={sigPosition}
											onValueChange={(v: "append" | "prepend") =>
												setSigPosition(v)
											}
										>
						...
						<div className="px-5 py-3 bg-muted/30 border-t border-border">
							<p className="text-[11px] text-muted-foreground">
								The default signature is automatically added to new posts. API
								users can skip it with{" "}
					...
					<div className="rounded-sm border border-border bg-card overflow-hidden">
							...
							<div className="divide-y divide-border">
								<div className="flex items-center justify-between px-5 py-3">
									<div className="flex-1 min-w-0 mr-4">
										...
										<p className="text-[11px] text-muted-foreground">
											When enabled, independent operational resources require an
											explicit{" "}
											<code className="text-[10px] bg-muted px-1 py-0.5 rounded">
												workspace_id
											</code>
											; parent-bound creates may inherit their parent workspace.
											When disabled, omission creates an organization-scoped
											resource for any credential with at least one workspace
											grant. Those resources are shared across non-empty grants;
											a zero-grant credential sees none. Shared definitions
											remain organization-wide in either mode.
										</p>
									</div>
									<Toggle
										enabled={requireWorkspaceId}
										onToggle={toggleRequireWorkspaceId}
										disabled={orgSettingsSaving || orgSettingsRevision === null}
									/>
								</div>
								{orgSettingsError && (
									<div className="px-5 py-3 bg-destructive/5 text-[11px] text-destructive">
										<p>{orgSettingsError}</p>
										{orgSettingsBlockers.length > 0 && (
											<ul className="mt-2 space-y-1 text-muted-foreground">
												{orgSettingsBlockers.map((blocker) => (
													<li key={blocker.resource_type}>
														{blocker.resource_type.replaceAll("_", " ")}:{" "}
														{blocker.count}
													</li>
												))}
											</ul>
						...
						<div className="px-5 py-3 bg-muted/30 border-t border-border">
							<p className="text-[11px] text-muted-foreground">
								Changes take effect after the database confirms the new settings
								revision.
							</p>
						</div>
					</div>
							...
							{!deleteConfirmOpen ? (
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-xs text-muted-foreground mt-0.5">
											Permanently delete this organization, all its API keys,
								...
								<div className="space-y-3">
									<p className="text-xs text-muted-foreground">
										...
										and all associated data including API keys, connections,
										posts, webhooks, and team members. This action cannot be
										undone.
									</p>
									<div className="space-y-1.5">
										<label
