...
export function AdminOrganizationsPage() {
	...
	const openEdit = (org: AdminOrg) => {
		...
		setEditForm({
			...
			slug: org.slug,
			dailyToolLimitOverride:
				org.dailyToolLimitOverride === null
					? ""
					: String(org.dailyToolLimitOverride),
		});
	};

	const saveEdit = async () => {
		if (!editModal) return;
		const trimmedToolLimit = editForm.dailyToolLimitOverride.trim();
		const dailyToolLimitOverride =
			trimmedToolLimit === "" ? null : Number(trimmedToolLimit);
		if (
			dailyToolLimitOverride !== null &&
			(!Number.isInteger(dailyToolLimitOverride) ||
				dailyToolLimitOverride < 0 ||
				dailyToolLimitOverride > 2_147_483_647)
		) {
			setError("Daily tool limit must be a nonnegative whole number");
			return;
		}
		setActionLoading(true);
		try {
	...
	const columns: ColumnDef<AdminOrg>[] = useMemo(
		() => [
			...
			{
				accessorKey: "dailyToolLimit",
				header: "Tools/day",
				cell: ({ row }) => (
					<span className="text-[13px] text-muted-foreground">
						{row.original.dailyToolLimit.toLocaleString()}
						{row.original.dailyToolLimitOverride !== null ? " custom" : ""}
					</span>
				),
			},
			{
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
					<div className="w-full max-w-sm rounded-[12px] border border-border bg-card p-6 space-y-4">
						...
						<div className="space-y-3">
							...
							<div className="space-y-1.5">
								<label
									htmlFor="admin-org-edit-daily-tool-limit"
									className="text-xs text-muted-foreground"
								>
									Daily tool limit override
								</label>
								<input
									id="admin-org-edit-daily-tool-limit"
									type="number"
									min="0"
									step="1"
									placeholder={`Plan default (${editModal.dailyToolLimit}/day)`}
									value={editForm.dailyToolLimitOverride}
									onChange={(e) =>
										setEditForm({
											...editForm,
											dailyToolLimitOverride: e.target.value,
										})
									}
									className="w-full rounded-[10px] border border-border bg-background px-3 py-2 text-sm outline-none transition-colors focus:border-foreground/20 focus:ring-2 focus:ring-ring/20"
								/>
								<p className="text-[11px] text-muted-foreground">
									Leave blank to inherit the current plan. Zero disables
									cost-bearing tools.
								</p>
							</div>
						</div>
