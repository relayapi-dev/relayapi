...
import { TOOL_PLATFORMS } from "@/lib/platform-char-limits";
...
export function PostValidatorDialog({
	open,
	onOpenChange,
}: PostValidatorDialogProps) {
	const [content, setContent] = useState("");
	const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>([]);
	const [result, setResult] = useState<ValidatePostResponse | null>(null);
	const { mutate, loading, error } = useMutation<ValidatePostResponse>(
		"tools/validate-post",
	);
	...
	const selectAll = () => {
		if (selectedPlatforms.length === TOOL_PLATFORMS.length) {
			setSelectedPlatforms([]);
		} else {
			setSelectedPlatforms(TOOL_PLATFORMS.map((p) => p.id));
		}
	};
			...
			<DialogContent className="sm:max-w-xl max-h-[85vh] grid-rows-[auto_1fr_auto] overflow-hidden">
					...
					<div className="space-y-3 pt-1 pl-1 pr-6">
						...
						<div className="space-y-1.5">
							<div className="flex items-center justify-between">
								...
								>
									{selectedPlatforms.length === TOOL_PLATFORMS.length
										? "Deselect all"
										: "Select all"}
								</button>
							</div>
						...
						{result && (
							<div className="space-y-2">
								...
								>
									...
									{result.valid
										? "Post is valid for all selected platforms"
										: "Post has validation issues"}
								</div>
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={
							!content.trim() || selectedPlatforms.length === 0 || loading
						}
					>
