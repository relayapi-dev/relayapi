...
import { TOOL_PLATFORM_LABELS } from "@/lib/platform-char-limits";
...
export function MediaValidatorDialog({
	open,
	onOpenChange,
}: MediaValidatorDialogProps) {
	const [url, setUrl] = useState("");
	const [result, setResult] = useState<ValidateMediaResponse | null>(null);
	const { mutate, loading, error } = useMutation<ValidateMediaResponse>(
							...
							<div className="space-y-3">
								...
								<ScrollArea className="max-h-[260px] rounded-md border border-border">
									<table className="w-full text-xs">
										...
										<tbody>
											{Object.entries(result.platform_limits).map(
												([platform, info]) => (
													<tr
														key={platform}
														className={`border-b border-border last:border-0 ${
															!info.within_limit ||
															info.mime_type_supported === false
																? "bg-destructive/5"
																: ""
														}`}
													>
														<td className="px-3 py-1.5 font-medium">
															{TOOL_PLATFORM_LABELS[platform] || platform}
														</td>
