...
export function TelegramDialog({
	...
	const initiate = useCallback(async () => {
		...
		try {
			const query = workspaceId
				...
				: "";
			const res = await fetch(`/api/connect/telegram${query}`);
			if (!res.ok) {
				const err = await res.json().catch(() => null);
				setStep({
					type: "error",
					message: err?.error?.message || "Failed to initiate connection",
				});
				return;
			}
			...
			pollRef.current = setInterval(async () => {
				try {
					const pollRes = await fetch(
						`/api/connect/telegram?code=${data.code}`,
					);
	...
	}, [onConnected, stopPolling, workspaceId]);
