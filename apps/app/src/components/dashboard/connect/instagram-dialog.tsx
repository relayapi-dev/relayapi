...
export function InstagramDialog({
	...
	const directParams = new URLSearchParams({ method: "direct" });
	const facebookParams = new URLSearchParams();
	if (workspaceId) {
		directParams.set("workspace_id", workspaceId);
		facebookParams.set("workspace_id", workspaceId);
	}
	const facebookQuery = facebookParams.toString();
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-md">
				...
				<div className="space-y-2 py-2">
					<a
						href={`/app/connect/start/instagram?${directParams}`}
					...
					<a
						href={`/app/connect/start/instagram${facebookQuery ? `?${facebookQuery}` : ""}`}
						className="flex items-center justify-between rounded-md border border-border p-3 transition-colors hover:bg-accent/20 no-underline text-inherit"
					>
						<div className="flex items-center gap-3">
							<div
							...
							<div>
								...
								<p className="text-xs text-muted-foreground">
									Connect via your linked Facebook account
								</p>
							</div>
						</div>
						<ArrowRight className="size-4 text-muted-foreground" />
