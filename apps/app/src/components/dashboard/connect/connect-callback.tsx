...
const SECONDARY_CONFIG: Record<
	string,
	{ endpoint: string; listKey: string; idField: string; label: string }
> = {
	facebook: {
		endpoint: "connect/facebook/pages",
		listKey: "pages",
		idField: "page_id",
		label: "Select a Page",
	},
	linkedin: {
		endpoint: "connect/linkedin/organizations",
		listKey: "organizations",
		idField: "organization_urn",
		label: "Select a Profile",
	},
	pinterest: {
		endpoint: "connect/pinterest/boards",
		listKey: "boards",
		idField: "board_id",
		label: "Select a Board",
	},
	googlebusiness: {
		endpoint: "connect/googlebusiness/locations",
		listKey: "locations",
		idField: "location_id",
		label: "Select a Location",
	},
	snapchat: {
		endpoint: "connect/snapchat/profiles",
		listKey: "profiles",
		idField: "profile_id",
		label: "Select a Profile",
	},
...
export function ConnectCallback({
	...
	const getInitialStep = (): Step => {
		...
		if (serverStatus === "success" && serverAccountId) {
			return serverWorkspaceId
				? { type: "success" }
				: { type: "assign-workspace", accountId: serverAccountId };
		}
	...
	const getAccountId = (
		data: { account?: { id?: string }; id?: string } | null | undefined,
	): string | null => {
	...
	const loadSecondaryItems = async () => {
		...
		if (!config) return;
		if (!serverConnectToken) {
			setStep({
				type: "error",
				message: "The account selection token is missing or expired",
			});
			return;
		}
		...
		try {
			const listRes = await fetch(
				`/api/${config.endpoint}?connect_token=${encodeURIComponent(serverConnectToken)}`,
			);
			if (listRes.ok) {
				...
				const items: SecondaryItem[] = rawItems.map((item) => ({
					...
					subtitle:
						...
						null,
					imageUrl:
						item.picture_url || item.logo_url || item.profile_image_url || null,
				}));
				...
				setStep({ type: "secondary", items, connectToken: serverConnectToken });
			} else {
	...
	useEffect(() => {
		...
		if (serverStatus === "success" && serverAccountId && serverWorkspaceId) {
			exchanged.current = true;
			goToSuccess(setStep);
			return;
		}
		...
		(async () => {
			try {
				...
				if (!res.ok) {
					...
					setStep({
						type: "error",
						message: err?.error?.message || `Connection failed (${res.status})`,
					});
					return;
				}
				...
				if (secondarySelectionPlatforms.has(platform)) {
					const config = SECONDARY_CONFIG[platform];
					const connectToken = responseData.connect_token;
					if (config && connectToken) {
						const listRes = await fetch(
							`/api/${config.endpoint}?connect_token=${encodeURIComponent(connectToken)}`,
						);
						if (listRes.ok) {
							const listData = await listRes.json();
							const rawItems: RawSecondaryItem[] =
								listData[config.listKey] || [];
							const items: SecondaryItem[] = rawItems.map((item) => ({
								id: item.id || item.urn || item.profile_id || "",
								name: item.name || item.display_name || "Unknown",
								subtitle:
				...
				if (accountId) {
					if (serverWorkspaceId) goToSuccess(setStep);
					else setStep({ type: "assign-workspace", accountId });
				} else {
					goToSuccess(setStep);
				}
			} catch (err) {
	...
	const handleSecondarySelect = async () => {
		...
		try {
			...
			if (!res.ok) {
				...
				setStep({
					type: "error",
					message: err?.error?.message || "Selection failed",
				});
				return;
			}
			...
			setStep({
				type: "error",
				message: err instanceof Error ? err.message : "Selection failed",
			});
		} finally {
			setSubmitting(false);
		}
				...
				<div className="flex flex-col items-center gap-4">
					...
					{step.type === "secondary" && (
						<div className="w-full">
							<div className="text-center mb-4">
								<h2 className="text-base font-medium">
									{SECONDARY_CONFIG[platform]?.label}
								</h2>
								<p className="mt-1 text-sm text-muted-foreground">
									Choose which{" "}
									{platform === "facebook"
