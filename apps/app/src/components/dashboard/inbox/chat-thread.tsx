...
function parseStoryMeta(platformData: unknown): {
	...
	const type =
		...
		STORY_MESSAGE_TYPES.has(data.message_type as StoryMessageType)
			? (data.message_type as StoryMessageType)
			: null;
	const url =
		typeof data.story_url === "string" && data.story_url.length > 0
			? data.story_url
			: null;
		...
		<div className="flex h-full items-center justify-center px-6">
			<div className="max-w-xs text-center">
				...
				<p className="mt-2 text-[13px] leading-6 text-muted-foreground">
					Pick a chat from the list to read and reply across all your connected
					channels.
				</p>
			</div>
		</div>
...
export function ChatThread({
	...
	const [automationControls, setAutomationControls] = useState<
		AutomationContactControlItem[]
	>([]);
	const [automationControlPending, setAutomationControlPending] =
		useState(false);
		...
		(async () => {
			try {
				...
				const json = (await res.json()) as {
					data?: AutomationContactControlItem[];
				};
				if (!cancelled) {
				...
				if (!cancelled) {
					setAutomationControlError(
						err instanceof Error
							? err.message
							: "Failed to load automation state",
					);
					setAutomationControls([]);
				}
			}
	...
	const fetchMessagesSilently = useCallback(async () => {
		...
		try {
			...
			const json = await res.json();
			const fresh = (json.messages || json.data || []).map(
				mapApiMessage,
			) as MessageItem[];
			...
			setMessages((prev) => {
				...
				const newMessages = fresh.filter(
					(message) =>
						!existingIds.has(message.id) && !message.id.startsWith("temp-"),
				);
	...
	const handleSend = async ({
		...
		try {
			...
			if (!res.ok) {
				...
				setSendError(
					json?.error?.message || json?.error || "Failed to send message",
				);
				return;
			}
		...
		try {
			...
			const res = await fetch(
				`/api/contacts/${encodeURIComponent(conversation.contact_id)}/${paused ? "automation-resume" : "automation-pause"}`,
				{
					...
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify(
						paused ? {} : { pause_reason: "Paused from inbox thread" },
					),
				},
			);
			if (!res.ok) {
	...
	const [startAutomationsError, setStartAutomationsError] = useState<
		string | null
	>(null);
	...
	const assignedMember = members.find(
		(member) => member.user.id === conversation.assigned_user_id,
	);
	const assigneeLabel =
		assignedMember?.user.name?.trim() ||
							...
							>
								...
								{membersLoading ? (
									...
									<DropdownMenuRadioGroup
										value={assigneeValue}
										onValueChange={(value) =>
											void handleAssignmentSelect(value)
										}
									>
										<DropdownMenuRadioItem
							...
							<AnimatePresence initial={false}>
								{threadItems.map((item, index) => {
										...
										return (
											...
											>
												<NoteCard
													note={item.data}
													canDelete={
														item.data.actor_type === "dashboard_user" &&
														item.data.actor_id === currentUserId
													}
													onDelete={async () => {
														const res = await fetch(
														...
														if (res.ok) {
															setNotes((prev) =>
																prev.filter((n) => n.id !== item.data.id),
															);
														}
													}}
												/>
									...
									const showDayDivider =
										!previousMessage ||
										dayKey(previousMessage.created_at) !==
