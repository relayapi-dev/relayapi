...
export const commentCapabilities: Record<
	string,
	{ hide: boolean; like: boolean; privateReply: boolean }
> = {
...
export function getConversationDisplayName(
	conversation: Pick<
		ConversationItem,
		"platform" | "participant_name" | "participant_metadata"
	>,
): string {
	const platform = conversation.platform?.toLowerCase();
	const instagramUsername =
		conversation.participant_metadata?.instagramProfile?.username?.trim();
...
export function getInstagramScopedId(
	conversation: Pick<
		ConversationItem,
		"platform" | "participant_name" | "participant_metadata"
	>,
	...
	const metadataScopedId =
		conversation.participant_metadata?.instagramProfile?.scopedId?.trim();
	if (metadataScopedId) {
		return metadataScopedId;
	}

	const participantName = conversation.participant_name?.trim();
	return participantName && /^\d+$/.test(participantName)
		? participantName
		: null;
}
