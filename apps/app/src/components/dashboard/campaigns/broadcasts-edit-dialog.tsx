...
export function BroadcastsEditDialog({
	...
	const isWhatsApp = detail?.platform === "whatsapp";
	const canDelete =
	...
	const handleSave = useCallback(async () => {
		...
		if (!isWhatsApp && !messageText.trim()) {
		...
		try {
			const res = await fetch(`/api/broadcasts/${broadcast.id}`, {
				...
				body: JSON.stringify({
					...
					...(!isWhatsApp ? { message_text: messageText.trim() } : {}),
				}),
			});
					...
					<div className="space-y-3 py-2 pl-0.5 pr-6">
						{loadingDetail ? (
							...
							<>
								...
								{/* Message or WhatsApp template */}
								<div>
									<label
										htmlFor="bc-edit-message"
										className="text-xs font-medium text-muted-foreground"
									>
										{isWhatsApp ? "Template" : "Message"}
									</label>
									{isWhatsApp ? (
										<div className="mt-1 rounded-md border border-border bg-accent/20 px-3 py-2 text-sm text-muted-foreground">
					...
					<div className="flex gap-2">
						...
						{isDraft && (
							<Button
								...
								onClick={handleSave}
								disabled={
									busy || loadingDetail || (!isWhatsApp && !messageText.trim())
								}
							>
								{saving ? (
									<Loader2 className="size-3.5 animate-spin" />
