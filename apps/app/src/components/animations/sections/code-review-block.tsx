...
const codeBlock = {
    fileName: "publish.ts",
    imports: [
        'const relay = new RelayClient("rlay_live_...");',
        "",
    ],
    before: [
        'await twitter.post({ text: message });',
        'await linkedin.post({ text: message });',
        'await instagram.post({ text: message });',
        '// ...repeat for each platform',
    ],
    after: [
        'await relay.posts.create({',
        '  text: message,',
        '  platforms: ["twitter", "linkedin", "instagram"],',
        '});',
    ],
    rest: [
        "// Response: { id: 'post_abc', status: 'published' }",
        "// All platforms handled in one call",
        "",
    ],
};
...
const ApplyButton = memo(function ApplyButton({ text, isClicked }: { text: string; isClicked: boolean }) {
    ...
    return (
        <motion.button
            ...
            className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 rounded-full bg-linear-to-b from-[#404040]/40 to-[#404040] text-secondary-foreground border border-card-foreground/20 h-12 w-fit px-6 text-sm font-medium backdrop-blur-2xl shadow-[0px_39px_16px_rgba(0,0,0,0.01),0px_22px_13px_rgba(0,0,0,0.05),0px_10px_10px_rgba(0,0,0,0.09),0px_2px_5px_rgba(0,0,0,0.1)] whitespace-nowrap"
        >
            {text}
        </motion.button>
    );
