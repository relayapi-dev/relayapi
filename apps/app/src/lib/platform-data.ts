...
export const platforms: PlatformData[] = [
	...
	},
	{
		...
		heroDescription:
			"The Snapchat Public Profile API requires an allowlisted OAuth client, explicit Public Profile selection, and Snap-specific encrypted media uploads. RelayAPI handles that connection and publishing flow.",
		directApiName: "Snapchat Public Profile API",
		painPoints: [
			"Public Profile API access requires Snapchat to allowlist the OAuth client",
			"OAuth authorization can expose several Public Profiles, so the connection must bind one verified profile identity",
			"Media creation requires encryption, bounded multipart upload, and provider finalization before content creation",
			"Saved Story and Spotlight have different payloads and terminal-result guarantees",
			"A Story create response has no correlation-safe status lookup, so RelayAPI rejects that path instead of guessing",
		],
		solutions: [
			"RelayAPI runs OAuth with snapchat-profile-api and lets you select only a Public Profile returned by Snapchat",
			"The selected Public Profile becomes the immutable account identity for every publish request",
			"RelayAPI performs the encrypted media upload and returns a Saved Story or Spotlight provider ID",
			"Spotlight submissions are polled to a moderation-aware pending, published, or failed result",
			"Unsupported Story requests fail before upload, avoiding ambiguous duplicate publication",
		],
		savingText: "Save 3+ hours of development time",
		warningBanner: {
			title: "Allowlisted Public Profile Access Required",
			description:
				"RelayAPI publishes only to a connector-verified Snapchat Public Profile. Accounts connected before Public Profile verification was introduced must reconnect before publishing.",
		},
		contentTypes: ["Saved Stories", "Spotlight"],
		features: [
			{
				title: "Ship Faster",
				description:
					"Publish a Saved Story or submit a Spotlight with one RelayAPI request while RelayAPI handles Snapchat's encrypted media-upload sequence.",
			},
			{
				title: "Verified Profile Identity",
				description:
					"RelayAPI uses the official Public Profile API and never lets a publish-time option redirect a token to a different profile.",
			},
			{
				title: "Truthful Outcomes",
				description:
					"Saved Stories require a returned provider ID, while Spotlight moderation is polled. The unsupported Story path is rejected before any upload.",
			},
		],
		faq: [
			{
				question: "What Snapchat access do I need?",
				answer:
					"The OAuth client must be allowlisted for Snapchat's Public Profile API, and the authorizing user must be able to select the Public Profile you want to connect.",
			},
			{
				question:
					"What content types can I publish to Snapchat through RelayAPI?",
				answer:
					"RelayAPI supports Saved Stories and Spotlight submissions. The ordinary Story path and Snapchat ad creatives are not part of this publisher contract.",
			},
			{
				question: "Why must older Snapchat connections reconnect?",
				answer:
					"Older records may contain a Marketing API organization or account ID rather than a verified Public Profile. RelayAPI fails those records closed with SNAPCHAT_RECONNECT_REQUIRED instead of assuming the identities are interchangeable.",
			},
			{
				question: "Can I post to Snapchat Spotlight through RelayAPI?",
				answer:
					"Yes. Spotlight requires exactly one video. RelayAPI preserves the request and Spotlight IDs and reports the provider's moderation-aware outcome.",
			},
		],
	},
	{
