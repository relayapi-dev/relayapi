...
const PLATFORM_SCOPE_MAP: Record<string, ScopeInfo[]> = {
	...
	threads: [
		{ scope: "threads_basic", label: "threads_basic", category: "posting" },
		{
			...
			label: "threads_content_publish",
			category: "posting",
		},
		{
			scope: "threads_manage_insights",
			label: "threads_manage_insights",
			category: "analytics",
		},
	],
	snapchat: [
...
export function categorizeScopeList(
	...
	const result = {
		posting: [] as ScopeInfo[],
		analytics: [] as ScopeInfo[],
		optional: [] as ScopeInfo[],
	};
...
export function getExpectedScopes(platform: string): {
	posting: ScopeInfo[];
	analytics: ScopeInfo[];
	optional: ScopeInfo[];
} {
...
export function hasPostingCapability(
	platform: string,
	grantedScopes: string[],
): boolean {
	const map = PLATFORM_SCOPE_MAP[platform];
