...
import { userAvatarPrefix } from "../../../lib/avatar-object-keys";
...
export const POST: APIRoute = async (ctx) => {
	...
	const prefix = userAvatarPrefix(user.id);
...
export const DELETE: APIRoute = async (ctx) => {
	...
	const existing = await bucket.list({ prefix: userAvatarPrefix(user.id) });
	if (existing.objects.length > 0) {
		await bucket.delete(existing.objects.map((o: MediaObject) => o.key));
	}
