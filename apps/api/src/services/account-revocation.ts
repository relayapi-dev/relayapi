...
export async function processAccountRevocations(env: Env): Promise<void> {
	...
	const due = await db
		.select({ id: accountRevocationJobs.id })
		...
		.where(
			and(
				...
				or(
					eq(accountRevocationJobs.status, "pending"),
					eq(accountRevocationJobs.status, "retry"),
					lte(accountRevocationJobs.leaseExpiresAt, now),
				),
			),
		)
		.orderBy(accountRevocationJobs.nextAttemptAt, accountRevocationJobs.id)
		.limit(20);
	...
	for (const candidate of due) {
		...
		if (!claimedLeaseExpiresAt) continue;
		...
		const result = token
			? await revokeProviderToken(env, job.platform, token).catch(
			...
			await db
				...
				.where(
					and(
						...
						eq(accountRevocationJobs.leaseExpiresAt, claimedLeaseExpiresAt),
					),
				);
			continue;
		}
		...
		const disconnected = await db.transaction(async (tx) => {
			...
			const clearedAccounts = await tx
				...
				.where(
					and(
						eq(socialAccounts.id, job.accountId),
						eq(socialAccounts.tokenVersion, job.sourceTokenVersion),
						eq(socialAccounts.lifecycleStatus, "disconnecting"),
					),
				)
			...
			await tx.insert(connectionLogs).values({
				...
				message:
					result.kind === "succeeded"
						? "Provider credential revoked and local disconnect completed"
						: `Local disconnect completed; provider cleanup requires manual action: ${result.reason}`,
				snapshot: {
