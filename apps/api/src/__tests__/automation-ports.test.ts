...
describe("derivePorts", () => {
	...
	test("message without wait_for_reply has no no_response port", () => {
		...
		expect(ports.map((p) => p.key)).toEqual(["in", "next", "error"]);
	});

	test("interactive message timeout derives no_response without an explicit wait flag", () => {
		const ports = derivePorts({
			kind: "message",
			config: {
				blocks: [
	...
	test("action_group has error only when any action has on_error=abort", () => {
		const withAbort = derivePorts({
			kind: "action_group",
			config: {
				actions: [{ id: "a", type: "tag_add", tag: "x", on_error: "abort" }],
			},
		});
		...
		const allContinue = derivePorts({
			kind: "action_group",
			config: {
				actions: [{ id: "a", type: "tag_add", tag: "x", on_error: "continue" }],
			},
		});
	...
	test("goto and end have no output ports", () => {
		expect(
			derivePorts({ kind: "goto", config: {} }).filter(
				(p) => p.direction === "output",
			),
		).toEqual([]);
		expect(
			derivePorts({ kind: "end", config: {} }).filter(
				(p) => p.direction === "output",
			),
		).toEqual([]);
	});
