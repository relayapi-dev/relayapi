...
export const CreateContentTemplateBody = z
	.object({
		...
		content: z
			...
			.min(1)
			.max(10000)
			.describe("Post text content (supports {{variables}})"),
		platform_overrides: z
			.record(PlatformEnum, z.string().min(1).max(10000))
			.optional()
...
export const UpdateContentTemplateBody = z
	.object({
...
export const ContentTemplateResponse = z.object({
	...
	workspace_id: z
		...
		.nullable()
		.describe(
			"Workspace ID for legacy scoped templates; newly created templates are organization-shared and return null",
		),
...
export const ContentTemplateListResponse = paginatedResponse(
